...
func (ic *ImageCreate) SetFilePath(s string) *ImageCreate {
	ic.mutation.SetFilePath(s)
	return ic
}

// AddArticleIDs adds the "article" edge to the Article entity by IDs.
func (ic *ImageCreate) AddArticleIDs(ids ...int) *ImageCreate {
	ic.mutation.AddArticleIDs(ids...)
	return ic
}

// AddArticle adds the "article" edges to the Article entity.
func (ic *ImageCreate) AddArticle(a ...*Article) *ImageCreate {
	ids := make([]int, len(a))
	for i := range a {
		ids[i] = a[i].ID
	}
	return ic.AddArticleIDs(ids...)
}
...
func (ic *ImageCreate) check() error {
	...
	if _, ok := ic.mutation.FilePath(); !ok {
		return &ValidationError{Name: "file_path", err: errors.New(`ent: missing required field "Image.file_path"`)}
	}
	if v, ok := ic.mutation.FilePath(); ok {
...
func (ic *ImageCreate) createSpec() (*Image, *sqlgraph.CreateSpec) {
	...
	if value, ok := ic.mutation.FilePath(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
