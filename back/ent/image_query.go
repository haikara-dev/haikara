...
func (iq *ImageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Image, error) {
	...
	if query := iq.withArticle; query != nil {
		if err := iq.loadArticle(ctx, query, nodes,
			func(n *Image) { n.Edges.Article = []*Article{} },
			func(n *Image, e *Article) { n.Edges.Article = append(n.Edges.Article, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *ImageQuery) loadArticle(ctx context.Context, query *ArticleQuery, nodes []*Image, init func(*Image), assign func(*Image, *Article)) error {
	...
	query.Where(predicate.Article(func(s *sql.Selector) {
		s.Where(sql.InValues(image.ArticleColumn, fks...))
	}))
