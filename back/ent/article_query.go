...
func (aq *ArticleQuery) QuerySite() *SiteQuery {
	query := &SiteQuery{config: aq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *ArticleQuery) WithSite(opts ...func(*SiteQuery)) *ArticleQuery {
	query := &SiteQuery{config: aq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *ArticleQuery) loadSite(ctx context.Context, query *SiteQuery, nodes []*Article, init func(*Article), assign func(*Article, *Site)) error {
	...
	nodeids := make(map[int][]*Article)
	for i := range nodes {
		if nodes[i].site_articles == nil {
			continue
		}
		fk := *nodes[i].site_articles
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "site_articles" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
