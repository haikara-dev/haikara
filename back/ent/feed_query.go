...
func (fq *FeedQuery) QuerySite() *SiteQuery {
	query := &SiteQuery{config: fq.config}
...
func (fq *FeedQuery) WithSite(opts ...func(*SiteQuery)) *FeedQuery {
	query := &SiteQuery{config: fq.config}
...
func (fq *FeedQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Feed, error) {
	...
	if query := fq.withSite; query != nil {
		if err := fq.loadSite(ctx, query, nodes, nil,
			func(n *Feed, e *Site) { n.Edges.Site = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FeedQuery) loadSite(ctx context.Context, query *SiteQuery, nodes []*Feed, init func(*Feed), assign func(*Feed, *Site)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Feed)
	for i := range nodes {
		if nodes[i].site_feeds == nil {
			continue
		}
		fk := *nodes[i].site_feeds
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "site_feeds" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
