...
type Client struct {
	...
	Feed *FeedClient
	// OGPImage is the client for interacting with the OGPImage builders.
	OGPImage *OGPImageClient
...
func (c *Client) init() {
	...
	c.OGPImage = NewOGPImageClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		OGPImage:      NewOGPImageClient(cfg),
...
func (c *ArticleClient) QueryOgpImage(a *Article) *OGPImageQuery {
	query := &OGPImageQuery{config: c.config}
...
// OGPImageClient is a client for the OGPImage schema.
type OGPImageClient struct {
	config
}

// NewOGPImageClient returns a client for the OGPImage from the given config.
func NewOGPImageClient(c config) *OGPImageClient {
	return &OGPImageClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `ogpimage.Hooks(f(g(h())))`.
func (c *OGPImageClient) Use(hooks ...Hook) {
	c.hooks.OGPImage = append(c.hooks.OGPImage, hooks...)
}

// Create returns a builder for creating a OGPImage entity.
func (c *OGPImageClient) Create() *OGPImageCreate {
	mutation := newOGPImageMutation(c.config, OpCreate)
	return &OGPImageCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of OGPImage entities.
func (c *OGPImageClient) CreateBulk(builders ...*OGPImageCreate) *OGPImageCreateBulk {
	return &OGPImageCreateBulk{config: c.config, builders: builders}
}

// Update returns an update builder for OGPImage.
func (c *OGPImageClient) Update() *OGPImageUpdate {
	mutation := newOGPImageMutation(c.config, OpUpdate)
	return &OGPImageUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *OGPImageClient) UpdateOne(oi *OGPImage) *OGPImageUpdateOne {
	mutation := newOGPImageMutation(c.config, OpUpdateOne, withOGPImage(oi))
	return &OGPImageUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *OGPImageClient) UpdateOneID(id int) *OGPImageUpdateOne {
	mutation := newOGPImageMutation(c.config, OpUpdateOne, withOGPImageID(id))
	return &OGPImageUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for OGPImage.
func (c *OGPImageClient) Delete() *OGPImageDelete {
	mutation := newOGPImageMutation(c.config, OpDelete)
	return &OGPImageDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *OGPImageClient) DeleteOne(oi *OGPImage) *OGPImageDeleteOne {
	return c.DeleteOneID(oi.ID)
}

// DeleteOne returns a builder for deleting the given entity by its id.
func (c *OGPImageClient) DeleteOneID(id int) *OGPImageDeleteOne {
	builder := c.Delete().Where(ogpimage.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &OGPImageDeleteOne{builder}
}

// Query returns a query builder for OGPImage.
func (c *OGPImageClient) Query() *OGPImageQuery {
	return &OGPImageQuery{
		config: c.config,
	}
}

// Get returns a OGPImage entity by its id.
func (c *OGPImageClient) Get(ctx context.Context, id int) (*OGPImage, error) {
	return c.Query().Where(ogpimage.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *OGPImageClient) GetX(ctx context.Context, id int) *OGPImage {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryArticle queries the article edge of a OGPImage.
func (c *OGPImageClient) QueryArticle(oi *OGPImage) *ArticleQuery {
	query := &ArticleQuery{config: c.config}
	query.path = func(ctx context.Context) (fromV *sql.Selector, _ error) {
		id := oi.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(ogpimage.Table, ogpimage.FieldID, id),
...
// Hooks returns the client hooks.
func (c *OGPImageClient) Hooks() []Hook {
	return c.hooks.OGPImage
}
